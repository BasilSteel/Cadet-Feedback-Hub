...
const Discussion = () => {
  ...
  const fetchDiscussions = async () => {
    try {
      ...
      const response = await fetch("http://localhost:4001/api/Discussion", {
        headers: {
  ...
  const handleCreateDiscussion = () => {
    const token = localStorage.getItem("token");
    fetch("http://localhost:4001/api/Discussion", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  const handleDeleteDiscussion = (discussionId) => {
    const token = localStorage.getItem("token");
    fetch(`http://localhost:4001/api/Discussion/${discussionId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
